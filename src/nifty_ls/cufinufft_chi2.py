...
def lombscargle(
    ...
    if dy.shape == ():  # Scalar case
        ...
        raise ValueError(f"Unsupported dy shape {dy.shape} for broadcast to ({Nbatch}, {N})")
    ...
    nSW = 2*nterms + 1
    nSY = nterms + 1
    Sw  = cp.empty((Nbatch, nSW, Nf), dtype=dtype) # Shape(Nbatch, nSW, Nf) and Initlize to 0
    ...
    for row_idx, (B_code, B_m) in enumerate(order):
        ...
        # Determine the operation type based on code combination
            ...
            if operation_type == 'CC':
                ...
                XTX[:, row_idx, col_idx, :] = 0.5 * (Cw[:, abs(A_m - B_m), :] + Cw[:, A_m + B_m, :])
            elif operation_type == 'SS':
                XTX[:, row_idx, col_idx, :] = 0.5 * (Cw[:, abs(A_m - B_m), :] - Cw[:, A_m + B_m, :])
            elif operation_type == 'CS':
                XTX[:, row_idx, col_idx, :] = 0.5 * (cp.sign(B_m - A_m) * Sw[:, abs(B_m - A_m), :] + Sw[:, B_m + A_m, :])
            elif operation_type == 'SC':
                XTX[:, row_idx, col_idx, :] = 0.5 * (cp.sign(A_m - B_m) * Sw[:, abs(A_m - B_m), :] + Sw[:, A_m + B_m, :])
            else:
    ...
    XTX_trans = XTX.transpose(0, 3, 1, 2).reshape(Nbatch * Nf, nterms_order, nterms_order)
